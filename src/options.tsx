...
function Options() {
  ...
  const content = (() => {
    ...
    if (section === "Permissões") return <><h1>Permissões</h1>
      ...
      {/* Quem instalou antes de estas duas virarem opcionais não passa de novo pelo pedido de acesso
          aos sites — então cada uma precisa do seu botão, ou ficaria sem jeito de concedê-la. */}
      <div className="settings-group">{(["notifications", "tabGroups"] as chrome.runtime.ManifestPermission[]).map((name) => <Row key={name} label={name} description={PERMISSION_LABELS[name]}>{permissions !== null && !permissions.includes(name)
        ? <button className="secondary-button" onClick={() => void chrome.permissions.request({ permissions: [name] }).then((feito) => { if (feito) setPermissions([...permissions, name]); }).catch(() => undefined)}>Conceder</button>
        : <span className={`connection-led led-${permissions === null ? "empty" : "ok"}`}><i />{permissions === null ? "—" : "concedida"}</span>}</Row>)}</div>
    </>;
