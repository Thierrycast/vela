...
const TOOLS = [
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        mode: { type: "string", enum: ["outline", "text"], description: "outline devolve a árvore de elementos interativos; text inclui o conteúdo legível." },
        offset: { type: "number", description: "Continuação de uma leitura truncada." },
        depth: { type: "number", description: "Até que profundidade descer na árvore da página. Padrão 12." },
        ref: { ...TEXT, description: "Relê só a subárvore deste elemento, em vez da página inteira." },
        tabId: { type: "number", description: "Aba a ler. Omita para a ativa." },
      },
    },
  },
  {
    name: "vela_act",
    description: "Age no Chrome do usuário. Os refs (como e412) vêm de vela_read_page ou de find e continuam valendo em releituras da mesma página — deixam de valer quando aquela aba navega. O modo de autonomia configurado na Vela continua valendo: em Observar a ação é recusada, e em Assistir ela espera aprovação humana no painel.",
    inputSchema: {
      type: "object",
      properties: {
        action: { type: "string", enum: ["navigate", "click", "type", "keyPress", "hover", "drag", "selectOption", "history", "scroll", "find", "screenshot", "waitFor", "wait", "pageTool", "evaluateScript"] },
        tabId: { type: "number", description: "Aba onde agir. Omita para a ativa; só abas da sessão da Vela são endereçáveis." },
        url: { ...TEXT, description: "Para navigate." },
        newTab: { type: "boolean" },
        ref: { ...TEXT, description: "Identificador do elemento, como e412. Diz sozinho em que aba agir." },
        selector: { ...TEXT, description: "Seletor CSS, quando não houver ref." },
        text: { ...TEXT, description: "Para type; e para waitFor, o texto esperado." },
        ...
        toRef: { ...TEXT, description: "Para drag: o elemento de destino." },
        toSelector: { ...TEXT, description: "Para drag: destino por seletor." },
        label: { ...TEXT, description: "Para selectOption: o texto da opção." },
        index: { type: "number", description: "Para selectOption: a posição da opção." },
        direction: { type: "string", enum: ["back", "forward"], description: "Para history." },
        query: { ...TEXT, description: "Para find: o que procurar. Aceita descrição por papel, como “campo de busca”." },
        role: { ...TEXT, description: "Para find: restringe ao papel (button, link, textbox...)." },
        gone: { type: "boolean", description: "Para waitFor: espera sumir em vez de aparecer." },
        networkIdle: { type: "boolean", description: "Para waitFor: espera a rede parar." },
        timeoutMs: { type: "number", description: "Para waitFor. Padrão 8000, teto 30000." },
        deltaX: { type: "number" },
        deltaY: { type: "number" },
        milliseconds: { type: "number", description: "Para wait." },
        depth: { type: "number", description: "Para extractPage via vela_read_page: profundidade da árvore." },
        script: { ...TEXT, description: "Para evaluateScript." },
        world: { type: "string", enum: ["isolated", "main"], description: "Para evaluateScript. main enxerga o JavaScript do site e precisa da habilidade ligada na Vela." },
